...
class Municipio
{
  ...
  //! \name Atributos privados de la clase Municipio
	...
	*/
    ...
    assert(getCodigoPostal()==codigoPostal);
    assert(getHombres()==hombres);
    assert(getMujeres()==mujeres);
	...
	*/
    ...
    assert(getCodigoPostal()==objeto.getCodigoPostal());
    assert(getHombres()==objeto.getHombres());
    assert(getMujeres()==objeto.getMujeres());
...
int getHabitantes() const
	{
    int valorDevuelto = getHombres() + getMujeres();
	...
	*/
	...
	/*!
		...
		\pre       Ninguna
		\post      getNombre() == municipio.getNombre()
...
//! \name Funciones auxiliares del espacio de nombres ed
	...
	/*!
		...
		\sa        operator<
	*/
	bool comparar (Municipio const &objeto1, Municipio const &objeto2) ;
