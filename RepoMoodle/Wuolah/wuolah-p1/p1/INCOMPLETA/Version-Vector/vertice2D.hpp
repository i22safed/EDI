...
class Vertice2D: public ed::vertice2DInterfaz
{
  ...
  //! \name Atributos privados de la clase Vertice2D
	...
	*/
	...
	/*! 
		...
		\note      Función inline
		\warning   Se requiere que las funciones de acceso getX y getY tengan el modificador const
		\param     v de tipo Vertice2D pasado como referencia constante
		\pre       El Vertice2D v debe existir y ser diferente del vertice actual
		\post      Ninguna
		\sa        getX(), getY(), setX(), setY()
	*/
	...
	inline bool operator == (Vertice2D const &v) const
  {
   ...
   //! \name Funciones amigas para poder acceder a la parte privada de la clase Vertice2D.

    /*!
		\fn        friend ostream &operator<<(ostream &stream, Vertice2D const &v);
		...
		\pre       El Vertice2D v debe existir
		\post      Se escriben por pantalla las coordenadas del vertice v de la forma (x, y)
		\sa        operator>>
    */
    friend ostream &operator<<(ostream &stream, Vertice2D const &v);

    /*!
		\fn         friend istream &operator>>(istream &stream, Vertice2D &v);
