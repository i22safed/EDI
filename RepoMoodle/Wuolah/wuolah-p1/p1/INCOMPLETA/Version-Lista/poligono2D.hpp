...
class Poligono2D: public ed::Poligono2DInterfaz
{
  //! \name Atributos privados de la clase Poligono2D
   private: 
	...
	std::list <Vertice2D> v_;//! lista que guarda los vertices que conforman el poligono
   ...
   public:
	...
	/*!
		\brief Crea un Poligono que almacena los vertices que lo forman en una lista 
		\note  Crea una lista que almacena datos de tipo Vertice2D
	 */
	inline Poligono2D (){};
	...
	/*!
		\brief Crea y opia un Poligono que almacena los vertices que lo forman en una lista 
		\note  Crea y copia una lista que almacena datos de tipo Vertice2D
		\param q Lista copia que vuelva su informacion en v_
	 */
	 
	inline Poligono2D (Poligono2D const &q){v_=q.v_;};
	
	 /*! 
		\name 	   DESTRUCTOR
		\brief     Destructor de la clase Poligono2D	       
		
		 Si el poligono está vacio no elimina los valores de la lista
	 	*/
	...
	/*!
		...
		\return devuelve un valor booleano que dice si esta o no vacia la lista
		
		Comprueba si la lista esta vacío o no, es decir, si tiene vertices o esta vacía y devuelve consecuentemente true o false si no esta vacía o si lo esta, respectivamente.
		*/
	...
	/*! 
		...
		\brief Te dice el número de vértices que tiene el poligono almacenado en la lista 
		\return devuelve una variable entera que te dice el numero de vertices que hay en la variable lista v_
		*/
	...
	/*! 
		...
		\param indice variable entera que te dice la posicion de la lista que quiere consultar 
	...
	inline bool existeVertice (const unsigned int &indice) const{return(indice<0 || indice>v_.size());};	
		...
		\return Devuelve el vertice que se quiere consultar de la lista pasando como variable local la posicion de la lista v_ en la que se encuentra.
		*/
	...
	@fn ed::Vertice2D const & operator [] (const int indice)
		\brief Accede al vertice almacenado en la posicion indicada en la lista
		\param indice variable entera que te dice la posicion de la lista que quiere consultar 
		\return Devuelve el vertice que se quiere consultar de la lista pasando como variable local la posicion de la lista v_ en la que se encuentra.
	...
	@fn void insertarVertice (const ed::Vertice2D &vertice)
		\brief Inserta un vertice al final de la lista, es decir, en la ultima posicion de este.
		\param vertice recibe como parametro el vertice que se quiere almacenar en la lista v_ 
		*/
	inline void insertarVertice (const ed::Vertice2D &vertice){v_.push_back(vertice);}
	
	/*!
	
	@fn void borrarVertice (int indice)
		\brief borra el vertice que ocupa la posicion indice de la lista v_. 
		\param indice es una variable enteera que indica la posicion de la lista en la que se encuentra el vertice que se quiera borrar
		 
		*/
	void borrarVertice (int indice);
	...
	*/
	...
	@fn void escribePoligono (ed::Poligono2D &poligono)
		\brief escribe en pantalla los vertices pertenecientes al poligono almacenados en la lista v_
	...
	*/
	...
	@fn void cargarPoligono (std::string nomfich)
		\brief carga un poligono desde un archivo de texto a la lista
   ...
   // DEBES CODIFICAR ESPECIFICAR LOS MÉTODOS DE LA CLASE Poligono2D
